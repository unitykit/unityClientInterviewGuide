...
1. 指针和引用在 C++ 中都用于间接访问变量，但它们有一些区别：
2. 指针是一个变量，它保存了另一个变量的内存地址；引用是另一个变量的别名，与原变量共享内存地址。
3. 指针(除指针常量)可以被重新赋值，指向不同的变量；引用在初始化后不能更改，始终指向同一个变量。
4. 指针可以为 nullptr，表示不指向任何变量；引用必须绑定到一个变量，不能为 nullptr。
5. 使用指针需要对其进行解引用以获取或修改其指向的变量的值；引用可以直接使用，无需解引用。
...
![img](assets/1733667301695-4eabc20d-c18e-4426-9cd4-be80d03d0d72.png)
...
![img](assets/1733667388460-dc2d21ea-fd11-42f1-9b17-a7200d505bdd.png)
...
1. 引用由编译器保证初始化，使用起来较为方便(如不用检查空指针等)
2. 尽量用引用代替指针
